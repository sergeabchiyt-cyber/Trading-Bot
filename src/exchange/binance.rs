...
impl BinanceClient {
    ...
    async fn signed_request(
        ...
        if let Some(code) = resp.get("code") {
            return Err(anyhow!("Binance error {}: {}", code, resp.get("msg").cloned().unwrap_or_default()));
        }
        Ok(resp)
    }

    pub async fn fetch_klines(&self, symbol: &str, interval: &str, limit: u32) -> Result<Vec<Vec<Value>>> {
        let url = format!(
    ...
    pub async fn place_order(&self, order: &super::types::OrderRequest) -> Result<Value> {
        ...
        self.signed_request(reqwest::Method::POST, "/fapi/v1/order", &query).await
    }

    pub async fn account_balance(&self) -> Result<Value> {
        self.signed_request(reqwest::Method::GET, "/fapi/v2/balance", "").await
    }

    pub async fn position_risk(&self, symbol: &str) -> Result<Value> {
        let params = format!("symbol={}", symbol);
        self.signed_request(reqwest::Method::GET, "/fapi/v2/positionRisk", &params).await
    }
}
